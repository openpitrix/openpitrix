...
func tCheckDefaultDB(t *testing.T) {
	...
	tAssert(t, cfg.DB.UserName == pb_default.GetDbRepoUserName())
	tAssert(t, cfg.DB.UserPassword == pb_default.GetDbRepoUserPassword())
}
...
func tChangeServiceHostAndPort() {
	os.Setenv("OPENPITRIX_CONFIG_REPO_HOST", "host")
	os.Setenv("OPENPITRIX_CONFIG_REPO_PORT", "1234")
}
func tCheckServiceHostAndPort(t *testing.T) {
	cfg, err := LoadConfig()
	tAssert(t, err == nil, err)

	tAssert(t, cfg.Repo.Host == "host")
	tAssert(t, cfg.Repo.Port == 1234)
}
