...
func (s *Server) SubmitVendorVerifyInfo(ctx context.Context, req *pb.SubmitVendorVerifyInfoRequest) (*pb.SubmitVendorVerifyInfoResponse, error) {
	...
	if ifExist {
		...
		attributes[constants.ColumnOwnerPath] = ownerPath
...
func (s *Server) DescribeAppVendorStatistics(ctx context.Context, req *pb.DescribeVendorVerifyInfosRequest) (*pb.DescribeVendorStatisticsResponse, error) {
	...
	for _, vendor := range vendors {
		...
		pbApps, appCnt, err := appClient.DescribeActiveAppsWithOwnerPath(ctx, accessPath, db.DefaultSelectLimit, 0)
		if err != nil {
			return nil, gerr.NewWithDetail(ctx, gerr.Internal, err, gerr.ErrorDescribeResourcesFailed)
		}

		//step2:if the real appCnt is smaller than db.DefaultSelectLimit(200),there is only one page apps,and the rows of this one page is length of pbApps.
		...
		if appCnt <= int32(db.DefaultSelectLimit) {
			...
			for i := 0; i < pages; i++ {
				...
				pbApps, _, err := appClient.DescribeActiveAppsWithOwnerPath(ctx, accessPath, db.DefaultSelectLimit, uint32(offset))
				if err != nil {
					return nil, gerr.NewWithDetail(ctx, gerr.Internal, err, gerr.ErrorDescribeResourcesFailed)
				}
