...
func TestLabelMapToString(t *testing.T) {
	labelStringMap := []map[string]string{
		{
			"runtime": "qingcloud",
			"zone":    "pk3a",
			"env":     "test",
	...
	for _, labelMap := range labelStringMap {
		labelString := LabelMapToString(labelMap)
		labelMap2, err := LabelStringToMap(labelString)
		if err != nil {
			t.Fatal()
		}
		if !reflect.DeepEqual(labelMap, labelMap2) {
			t.Fatal()
		}
	}
}
...
func TestLabelMapDiff(t *testing.T) {
	...
	for n, _ := range oldLabelMapArray {
