...
func (s *Server) CreateCombinationSku(ctx context.Context, req *pb.CreateCSRequest) (*pb.CreateCSResponse, error) {
	comSku := models.PbToComSku(req)

	//check if CRA exist
	err := checkStructExistById(ctx, models.CombinationResourceAttribute{}, comSku, comSku.CSId, CreateFailedCode)
	if err != nil {
		return nil, err
	}
	...
	//insert CS
	err = insertCS(ctx, comSku)
	if err != nil {
		return nil, commonInternalErr(ctx, comSku, CreateFailedCode)
	}
	return &pb.CreateCSResponse{CsId: pbutil.ToProtoString(comSku.CSId)}, nil
}

func (s *Server) CreateCombinationPrice(ctx context.Context, req *pb.CreateCPRequest) (*pb.CreateCPResponse, error) {
	comPrice := models.PbToComPrice(req)

	//check if com_sku exist
	err := checkStructExistById(ctx, models.CombinationSku{}, comPrice, comPrice.CSkuId, CreateFailedCode)
	if err != nil {
		return nil, err
	}
	...
	return &pb.CreateCPResponse{CpId: pbutil.ToProtoString(comPrice.CPId)}, nil
}
