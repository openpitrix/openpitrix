...
func tCheckDefultDB(t *testing.T) {
	...
	tAssert(t, cfg.DB.UserName == pb_default.GetDbRuntimeUserName())
	tAssert(t, cfg.DB.UserPassword == pb_default.GetDbRuntimeUserPassword())
}
...
func tChangeServiceHostAndPort() {
	os.Setenv("OPENPITRIX_CONFIG_RUNTIME_HOST", "host")
	os.Setenv("OPENPITRIX_CONFIG_RUNTIME_PORT", "1234")
}
func tCheckServiceHostAndPort(t *testing.T) {
	cfg, err := LoadConfig()
	tAssert(t, err == nil, err)

	tAssert(t, cfg.Runtime.Host == "host")
	tAssert(t, cfg.Runtime.Port == 1234)
}
