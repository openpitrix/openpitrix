...
func tCheckDefultDB(t *testing.T) {
	...
	tAssert(t, cfg.DB.UserName == pb_default.GetDbClusterUserName())
	tAssert(t, cfg.DB.UserPassword == pb_default.GetDbClusterUserPassword())
}
...
func tChangeServiceHostAndPort() {
	os.Setenv("OPENPITRIX_CONFIG_CLUSTER_HOST", "host")
	os.Setenv("OPENPITRIX_CONFIG_CLUSTER_PORT", "1234")
}
func tCheckServiceHostAndPort(t *testing.T) {
	cfg, err := LoadConfig()
	tAssert(t, err == nil, err)

	tAssert(t, cfg.Cluster.Host == "host")
	tAssert(t, cfg.Cluster.Port == 1234)
}
